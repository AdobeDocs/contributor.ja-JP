...
description: Experience Leagueに関するAdobeドキュメントに投稿する方法を説明します。
...
リコール元 [Git と GitHub の基本](git-fundamentals.md) Git リポジトリには、メインブランチと、メインに統合されていない作業中のブランチが追加されます。 論理的に関連する一連の変更を導入する場合は、ベストプラクティスとして *作業ブランチ* を使用して、ワークフロー内の変更を管理します。 ここでは、作業ブランチと呼びます。これは、変更をメインブランチに統合できるまで繰り返し処理し、調整するためのワークスペースなのです。
...
>[!NOTE]
>
>メインブランチでの変更 *良い習慣ではない*. 例えば、メインブランチを使用して、時間計測機能リリースに一連の変更を導入したとします。 あなたは変更作業を終えて、リリースを待っています。その前に、緊急の修正依頼があったので、メインブランチのファイルに変更を加え、変更をパブリッシュしました。 この場合は、最新の修正結果と、特定期日のリリースに導入した保留中の変更の&#x200B;*両方*&#x200B;が誤ってパブリッシュされてしまいます。
...
いくつかの変更案を新しいプルリクエスト（PR）にまとめて提出します。PR は、宛先リポジトリの PR キューに追加されます。プルリクエストは GitHub の共同作業モデルを動かすための要素です。プリリクエストを使用して、作業ブランチから変更をプルし、別のブランチにマージするよう依頼することができます。ほとんどの場合、メインリポジトリのデフォルト/メインブランチは他のブランチになります。
