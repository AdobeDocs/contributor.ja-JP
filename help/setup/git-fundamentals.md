...
- *ブランチ*:すべてのリポジトリには、デフォルトブランチ（一般的な名称は「main」）と、メインブランチにマージされる予定の 1 つ以上のブランチが含まれています。 メインブランチは、コンテンツのパブリッシュ元となる現在のバージョンおよびソースとして機能します。 このブランチは、リポジトリ内の他のすべてのブランチの親になります。
...
プロジェクトのデフォルト/メインブランチは、プロジェクトのコンテンツの現在のバージョンとして機能します。 メインブランチとそこから作成されたブランチのコンテンツは、記事のトピックの構成に従って編成されます。 コンテンツおよび画像アセットを編成するために、サブディレクトリが使用されます。
